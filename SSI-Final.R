#############################################################################
# Chose Your Own Project: Recommendation System for Google Playstore App
############################################################################
...
# List of all rows with Rating=NaN

x<-str_which(googleplaystore$Rating,"NaN") 

# Since other entries seem to be correct, at this stage we will not delete any of the rows with NaN as Rating
...
googleplaystore %>% filter(is.na(as.numeric(googleplaystore$Reviews))) 
pattern <- "^[A-z]"
sum(str_detect(googleplaystore$Size,pattern))

# So there are 9146 out of 10473 entries that  match with a size description

googleplaystore%>% group_by(Size)%>% summarise(count=n()) %>% arrange(desc(count)) 

# We see that the one is not a mistake but netry corresponds to "varies by device"

googleplaystore %>% group_by(Type)%>% summarise(count=n()) 

#There is one NaN and one 0, and the line with Type=0 is the same line that will be taken out when clearing for Reviews

s<-"NaN"

googleplaystore[str_which(googleplaystore$Type,s),]

#Since data is unrelevant, this line will be take out
...
str_which(googleplaystore$Installs,pattern_I)

googleplaystore[c(str_which(googleplaystore$Installs,pattern_I)),] 

#I see that genrally this App has wrong entries in nearly all columns

New_googleplaystore<-googleplaystore[-c(str_which(googleplaystore$Type,s),str_which(googleplaystore$Installs,pattern_I)),] 

#I took out the line with wrong inputs since it is also correlating the wrong Reviews

str(New_googleplaystore)

#Since for validation set and test set we need the ratings to be numerical and none Nan, 
#we will need to take out the NaN entries from the dataframe
# list of all rows with Rating =NaN
...
#798 Apps are repeated, below is the example of most frequently repated App

New_googleplaystore %>% filter(App== "Candy Crush Saga")

New_googleplaystore %>% filter(App %in% (l[1:798,1])) 

#Next step is to take out the duplicated App entris

New_googleplaystore<-New_googleplaystore %>% distinct(App, .keep_all = TRUE) 

#Changeing the variable type for Installs from chr to number 
...
# Finding non-regular App names and clear them out.
...
# Installs distribution plot by Reviews
...
# Installs distribution plot by Category
...
# Validation
# Validation set will be 10% of Google Play store data
...
temp <- New_googleplaystore[test_index,] 

# Validate that Category,Installs and Genres in validation set are also in Capstone set
...
# Top 10 App reviewed
print("Top 10 App reviewed")
Capstone %>% group_by(App, Reviews) %>%  arrange(desc(Reviews))

# Top 10 Apps least reviewed
print("Top 10 Apps least reviewed")
...
# Processing the first step of the Recomandation System by applying naively the mu_hat( average ratings of all App) and by this process the naive rmse
